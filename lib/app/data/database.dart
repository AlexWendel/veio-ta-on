  ...
  final CollectionReference<UserLocal> userConverter =
      FirebaseFirestore.instance.collection(userCollection).withConverter(
          fromFirestore: (snapshot, _) {
            ...
            // print(data.toString() + " " + snapshot.id);
            return UserLocal.fromJson(data, snapshot.id);
          },
          toFirestore: (UserLocal user, _) => user.toJson());

  Future<UserLocal> userReference(String reference) async {
    reference = reference.split('/')[1];
    return userConverter.doc(reference).get().then((snapshot) {
